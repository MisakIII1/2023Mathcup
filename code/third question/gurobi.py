...
y = np.array([model.addVar(vtype=GRB.BINARY, name=f'y_{i}') for i in range(M)])
z = np.array([[[[model.addVar(vtype=GRB.BINARY, name=f'z_{i}_{j}_{m}_{n}') for n in range(N)]
                for m in range(N)] for j in range(M)] for i in range(M)])

obj = QuadExpr(quicksum(t[i, m] * t[j, n] * t[k, o] * z[i, j, m, n] * x[k, o] * (((1 + I)/3) * (h[i, m] + h[j, n] + h[k, o]) - I)
...
model.addConstrs(x[i, m] * x[j, n] - z[i, j, m, n] == 0
