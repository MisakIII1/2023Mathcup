...
obj = P*QuadExpr(quicksum(t[i, m]*t[j, n]*t[k, o]*y[i, j, m, n]*x[k, o]*(((1+I)/3)*(h[i, m]+h[j, n]+h[k, o])-I)
                         for i in range(M) for j in range(i+1, M) for k in range(j+1, M)
                         for m in range(N) for n in range(N) for o in range(N))+
                 c*quicksum((quicksum(x[i])-1)**2 for i in range(M))+
                 c*quicksum(3*y[i, j, m, n]+x[i, m]*x[j, n]-2*x[i, m]*y[i, j, m, n]-2*x[j, n]*y[i, j, m, n]
